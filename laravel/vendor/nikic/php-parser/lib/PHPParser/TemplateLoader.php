...
class PHPParser_TemplateLoader {
	 ...
	 *        	The base directory to load templates from
	 * @param string $suffix
	 *        	An optional suffix to append after the template name
	 */
	public function __construct(PHPParser_Parser $parser, $baseDir, $suffix = '') {
		if (! is_dir ( $baseDir )) {
			throw new InvalidArgumentException ( sprintf ( 'The specified base directory "%s" does not exist', $baseDir ) );
		}
