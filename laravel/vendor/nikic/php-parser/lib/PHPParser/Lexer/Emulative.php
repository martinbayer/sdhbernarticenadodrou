...
class PHPParser_Lexer_Emulative extends PHPParser_Lexer {
	...
	protected function preprocessCode($code) {
		...
		if (version_compare ( PHP_VERSION, '5.3.0', '<' )) {
			...
			// nowdoc (<<<'ABC'\ncontent\nABC;)
			$code = preg_replace_callback ( '((*BSR_ANYCRLF)        # set \R to (?>\r\n|\r|\n)
	...
	public function encodeNowdocCallback(array $matches) {
		return '~__EMU__NOWDOC__' . bin2hex ( $matches [1] ) . 'x' . bin2hex ( $matches [3] ) . 'x' . bin2hex ( $matches [4] ) . '__~';
	}
	...
	protected function postprocessTokens() {
		...
		for($i = 0, $c = count ( $this->tokens ); $i < $c; ++ $i) {
			...
			if ('~' === $this->tokens [$i] && isset ( $this->tokens [$i + 2] ) && '~' === $this->tokens [$i + 2] && T_STRING === $this->tokens [$i + 1] [0] && preg_match ( '(^__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?$)', $this->tokens [$i + 1] [1], $matches )) {
				if ('BINARY' === $matches [1]) {
					// the binary number can either be an integer or a double, so return a LNUMBER
					// or DNUMBER respectively
					$replace = array (
							array (
									is_int ( bindec ( $matches [2] ) ) ? T_LNUMBER : T_DNUMBER,
									$matches [2],
									$this->tokens [$i + 1] [2] 
							) 
					);
			...
			} elseif (is_array ( $this->tokens [$i] ) && 0 !== strpos ( $this->tokens [$i] [1], '__EMU__' )) {
				$this->tokens [$i] [1] = preg_replace_callback ( '(~__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?~)', array (
						$this,
