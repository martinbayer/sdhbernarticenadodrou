...
class PHPParser_Unserializer_XML implements PHPParser_Unserializer {
	...
	protected function readNode() {
		...
		$node = unserialize ( sprintf ( "O:%d:\"%s\":2:{s:11:\"\0*\0subNodes\";a:0:{}s:13:\"\0*\0attributes\";a:0:{}}", strlen ( $className ), $className ) );
		...
		while ( $this->reader->read () && $depthLimit < $this->reader->depth ) {
			...
			if ('subNode' !== $type && 'attribute' !== $type) {
				throw new DomainException ( sprintf ( 'Expected sub node or attribute, got node of type "%s"', $this->reader->name ) );
			}
	...
	protected function readComment() {
		$className = $this->reader->getAttribute ( 'isDocComment' ) === 'true' ? 'PHPParser_Comment_Doc' : 'PHPParser_Comment';
		return new $className ( $this->reader->readString (), $this->reader->getAttribute ( 'line' ) );
	}
}
