...
class PHPParser_Tests_NodeTraverserTest extends PHPUnit_Framework_TestCase {
	public function testNonModifying() {
		...
		$echoNode = new PHPParser_Node_Stmt_Echo ( array (
				$str1Node,
				$str2Node 
		) );
		$stmts = array (
	...
	public function testModifying() {
		...
		$visitor1->expects ( $this->at ( 0 ) )->method ( 'beforeTraverse' )->with ( array () )->will ( $this->returnValue ( array (
				$str1Node 
		) ) );
		$visitor2->expects ( $this->at ( 0 ) )->method ( 'beforeTraverse' )->with ( array (
				$str1Node 
		) );
		...
		$visitor1->expects ( $this->at ( 5 ) )->method ( 'afterTraverse' )->with ( array (
				$str1Node 
	...
	public function testMerge() {
		...
		$visitor->expects ( $this->at ( 4 ) )->method ( 'leaveNode' )->with ( $strMiddle )->will ( $this->returnValue ( array (
				$strR1,
				$strR2 
		) ) );
