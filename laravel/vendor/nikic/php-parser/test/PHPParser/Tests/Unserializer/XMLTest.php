...
XML;
		...
		$this->assertEquals ( new PHPParser_Node_Scalar_String ( 'Test', array (
				'startLine' => 1,
				'comments' => array (
...
XML;
		...
		$this->assertEquals ( new PHPParser_Node_Scalar_ClassConst (), $unserializer->unserialize ( $xml ) );
	}
...
XML;
	...
	public function provideTestErrors() {
		return array (
				array (
						'<scalar:true>test</scalar:true>',
						'"true" scalar must be empty' 
				),
				array (
						'<scalar:false>test</scalar:false>',
						'"false" scalar must be empty' 
				),
				array (
						'<scalar:null>test</scalar:null>',
						'"null" scalar must be empty' 
				),
				array (
						'<scalar:foo>bar</scalar:foo>',
						'Unknown scalar type "foo"' 
				),
				array (
						'<scalar:int>x</scalar:int>',
						'"x" is not a valid int' 
				),
				array (
						'<scalar:float>x</scalar:float>',
				...
				array (
						...
						'Unexpected node of type "foo:bar"' 
				),
				array (
