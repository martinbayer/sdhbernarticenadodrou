...
class PHPParser_Tests_Builder_ClassTest extends PHPUnit_Framework_TestCase {
	...
	public function testExtendsImplements() {
		$node = $this->createClassBuilder ( 'SomeLogger' )->extend ( 'BaseLogger' )->implement ( 'Namespaced\Logger', new PHPParser_Node_Name ( 'SomeInterface' ) )->getNode ();
		
		$this->assertEquals ( new PHPParser_Node_Stmt_Class ( 'SomeLogger', array (
	...
	public function testAbstract() {
		...
		$this->assertEquals ( new PHPParser_Node_Stmt_Class ( 'Test', array (
	...
	public function testStatementOrder() {
		...
		$property = new PHPParser_Node_Stmt_Property ( PHPParser_Node_Stmt_Class::MODIFIER_PUBLIC, array (
		...
		$use = new PHPParser_Node_Stmt_TraitUse ( array (
				new PHPParser_Node_Name ( 'SomeTrait' ) 
		) );
		
		$node = $this->createClassBuilder ( 'Test' )->addStmt ( $method )->addStmt ( $property )->addStmts ( array (
				$const,
				$use 
		) )->getNode ();
