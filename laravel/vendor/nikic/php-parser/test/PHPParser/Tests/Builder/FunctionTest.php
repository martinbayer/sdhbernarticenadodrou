...
class PHPParser_Tests_Builder_FunctionTest extends PHPUnit_Framework_TestCase {
	...
	public function testReturnByRef() {
		...
		$this->assertEquals ( new PHPParser_Node_Stmt_Function ( 'test', array (
				'byRef' => true 
		) ), $node );
	}
	public function testParams() {
		...
		$node = $this->createFunctionBuilder ( 'test' )->addParam ( $param1 )->addParams ( array (
				$param2,
				$param3 
		) )->getNode ();
	...
	public function testStmts() {
		...
		$node = $this->createFunctionBuilder ( 'test' )->addStmt ( $stmt1 )->addStmts ( array (
				$stmt2,
				$stmt3 
		) )->getNode ();
	...
	public function testInvalidParamError() {
		$this->createFunctionBuilder ( 'test' )->addParam ( new PHPParser_Node_Name ( 'foo' ) );
	}
}