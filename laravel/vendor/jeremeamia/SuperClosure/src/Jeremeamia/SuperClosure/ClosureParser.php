...
/**
 * Parses a closure from its reflection such that the code and used (closed upon) variables are accessible.
 * The
...
class ClosureParser {
	...
	/**
	 * Returns the abstract syntax tree (AST) of the closure's code.
	 * Class names are resolved to their fully-qualified
	...
	public function getClosureAbstractSyntaxTree() {
		if (! $this->abstractSyntaxTree) {
			...
			$closureAst = $closureTraverser->traverse ( array (
					$closureAst 
			) );
	 ...
	 * These are referred to as the "used
	...
	public function getCode() {
		if (! $this->code) {
			...
			$this->code = $printer->prettyPrint ( array (
					$this->getClosureAbstractSyntaxTree () 
			) );
		}
	...
	/**
	 * Loads the PHP file and produces an abstract syntax tree (AST) of the code.
	 * This is stored in an internal cache by
