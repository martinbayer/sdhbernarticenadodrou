...
class ConnectionFactory implements ConnectionFactoryInterface {
	 ...
	 *
	 * @param ServerProfileInterface $profile
	 *        	Server profile used to initialize new connections.
	 */
	 ...
	 * implementing Predis\Connection\SingleConnectionInterface.
	 * Optionally,
	...
	protected function prepareConnection(SingleConnectionInterface $connection) {
		if (isset ( $this->profile )) {
			...
			if (isset ( $parameters->password )) {
				$command = $this->profile->createCommand ( 'auth', array (
						$parameters->password 
				) );
			...
			if (isset ( $parameters->database )) {
				$command = $this->profile->createCommand ( 'select', array (
						$parameters->database 
				) );
				$connection->pushInitCommand ( $command );
			}
