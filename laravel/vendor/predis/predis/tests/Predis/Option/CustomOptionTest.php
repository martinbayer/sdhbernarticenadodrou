...
class CustomOptionTest extends PredisTestCase {
	...
	public function testInvokeCallsFilterCallback() {
		...
		$filter = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
		$filter->expects ( $this->once () )->method ( '__invoke' )->with ( $this->isInstanceOf ( 'Predis\Option\ClientOptionsInterface' ), $value )->will ( $this->returnValue ( true ) );
		
		$default = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
	...
	public function testInvokeCallsDefaultCallback() {
		...
		$default->expects ( $this->once () )->method ( '__invoke' )->with ( $this->isInstanceOf ( 'Predis\Option\ClientOptionsInterface' ) )->will ( $this->returnValue ( true ) );
