...
class ClientConnectionFactoryTest extends PredisTestCase {
	...
	public function testValidationReturnsDefaultFactoryWithSchemeDefinitionsArray() {
		...
		$default->expects ( $this->exactly ( 2 ) )->method ( 'define' )->with ( $this->matchesRegularExpression ( '/^tcp|redis$/' ), $connectionClass );
		
		$option = $this->getMock ( 'Predis\Option\ClientConnectionFactory', array (
				'getDefault' 
		) );
		$option->expects ( $this->once () )->method ( 'getDefault' )->with ( $options )->will ( $this->returnValue ( $default ) );
	...
	public function testValidationAcceptsCallableObjectAsInitializers() {
		...
		$initializer = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
	...
	public function testInvokeReturnsSpecifiedFactoryOrDefault() {
		...
		$option->expects ( $this->once () )->method ( 'filter' )->with ( $options, $value )->will ( $this->returnValue ( $value ) );
		$option->expects ( $this->never () )->method ( 'getDefault' );
		...
		$option->expects ( $this->once () )->method ( 'getDefault' )->with ( $options )->will ( $this->returnValue ( $value ) );
