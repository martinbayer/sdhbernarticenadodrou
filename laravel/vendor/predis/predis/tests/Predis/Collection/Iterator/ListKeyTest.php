...
class ListKeyTest extends PredisTestCase {
	...
	public function testIterationWithNoResults() {
		...
		$client->expects ( $this->once () )->method ( 'lrange' )->with ( 'key:list', 0, 9 )->will ( $this->returnValue ( array () ) );
	...
	public function testIterationOnMultipleFetches() {
		...
		$client->expects ( $this->any () )->method ( 'getProfile' )->will ( $this->returnValue ( ServerProfile::getDefault () ) );
		$client->expects ( $this->at ( 1 ) )->method ( 'lrange' )->with ( 'key:list', 0, 9 )->will ( $this->returnValue ( array (
				'item:1',
				'item:2',
				'item:3',
				'item:4',
		...
		$client->expects ( $this->at ( 2 ) )->method ( 'lrange' )->with ( 'key:list', 10, 19 )->will ( $this->returnValue ( array (
				'item:11',
				'item:12' 
		) ) );
	...
	public function testIterationWithCountParameter() {
		...
		$client->expects ( $this->any () )->method ( 'getProfile' )->will ( $this->returnValue ( ServerProfile::getDefault () ) );
		$client->expects ( $this->at ( 1 ) )->method ( 'lrange' )->with ( 'key:list', 0, 4 )->will ( $this->returnValue ( array (
				'item:1',
				'item:2' 
		) ) );
	...
	public function testIterationWithCountParameterOnMultipleFetches() {
		...
		$client->expects ( $this->any () )->method ( 'getProfile' )->will ( $this->returnValue ( ServerProfile::getDefault () ) );
		$client->expects ( $this->at ( 1 ) )->method ( 'lrange' )->with ( 'key:list', 0, 1 )->will ( $this->returnValue ( array (
				'item:1',
				'item:2' 
		) ) );
		$client->expects ( $this->at ( 2 ) )->method ( 'lrange' )->with ( 'key:list', 2, 3 )->will ( $this->returnValue ( array (
				'item:3' 
		) ) );
	...
	public function testIterationRewindable() {
		...
		$client->expects ( $this->any () )->method ( 'getProfile' )->will ( $this->returnValue ( ServerProfile::getDefault () ) );
		$client->expects ( $this->exactly ( 2 ) )->method ( 'lrange' )->with ( 'key:list', 0, 9 )->will ( $this->returnValue ( array (
				'item:1',
				'item:2' 
		) ) );
