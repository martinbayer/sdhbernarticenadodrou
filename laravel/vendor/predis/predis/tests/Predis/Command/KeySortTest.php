...
class KeySortTest extends PredisCommandTestCase {
	...
	public function testBasicSort() {
		...
		$redis->lpush ( 'list:unordered', $unordered = array (
				2,
				100,
				3,
				1,
	...
	public function testSortWithStoreModifier() {
		...
		$this->assertEquals ( count ( $unordered ), $redis->sort ( 'list:unordered', array (
				'store' => 'list:ordered' 
		) ) );
	...
	public function testSortWithGetModifiers() {
		...
		$redis->rpush ( 'list:uids', $uids = array (
				1003,
				1001,
				1002,
				1000 
		...
		$this->assertEquals ( array_values ( $sortget ), $redis->sort ( 'list:uids', array (
				'get' => 'uid:*' 
		) ) );
	}
