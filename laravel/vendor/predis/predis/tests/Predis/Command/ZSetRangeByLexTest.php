...
class ZSetRangeByLexTest extends PredisCommandTestCase {
	...
	public function testReturnsElementsInInclusiveRange() {
		...
		$this->assertSame ( array (
				'a' 
		), $redis->zrangebylex ( 'letters', '[a', '[a' ) );
		$this->assertSame ( array (
				'c',
				'd',
				'e',
				'f' 
		), $redis->zrangebylex ( 'letters', '[c', '[f' ) );
		$this->assertSame ( array (
				'a',
				'b',
				'c' 
		), $redis->zrangebylex ( 'letters', '-', '[c' ) );
	...
	public function testRangeWithLimitModifier() {
		...
		$this->assertSame ( array (
				...
				'd',
				'e' 
		), $redis->zrangebylex ( 'letters', '-', '+', 'LIMIT', '2', '3' ) );
		$this->assertSame ( array (
				'c',
				'd',
				'e' 
		), $redis->zrangebylex ( 'letters', '-', '+', array (
				'limit' => array (
						2,
						3 
				) 
