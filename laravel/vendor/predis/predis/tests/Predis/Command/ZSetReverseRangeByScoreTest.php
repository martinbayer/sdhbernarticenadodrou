...
class ZSetReverseRangeByScoreTest extends PredisCommandTestCase {
	...
	public function testParseResponseWithScores() {
		...
		$command = $this->getCommandWithArgumentsArray ( array (
				'zset',
				0,
				1,
				'withscores' 
	...
	public function testAddsWithscoresModifiersOnlyWhenOptionIsTrue() {
		$command = $this->getCommandWithArguments ( 'zset', 0, 100, array (
				'withscores' => true 
		) );
	...
	public function testReturnsElementsInScoreRange() {
		...
		$this->assertSame ( array (
				'a' 
		), $redis->zrevrangebyscore ( 'letters', - 10, - 10 ) );
		$this->assertSame ( array (), $redis->zrevrangebyscore ( 'letters', 10, 30 ) );
		$this->assertSame ( array (
				'e',
				'd' 
		), $redis->zrevrangebyscore ( 'letters', 20, 20 ) );
		$this->assertSame ( array (
				'f',
				'e',
				'd',
				'c',
				'b' 
		), $redis->zrevrangebyscore ( 'letters', 30, 0 ) );
	...
	public function testInfinityScoreIntervals() {
		...
		$this->assertSame ( array (
				...
				'e',
				'd' 
		), $redis->zrevrangebyscore ( 'letters', '+inf', 15 ) );
		$this->assertSame ( array (
				'c',
				'b',
				'a' 
		), $redis->zrevrangebyscore ( 'letters', 15, '-inf' ) );
		$this->assertSame ( array (
				'f',
				'e',
				'd',
				...
				'b',
				'a' 
		), $redis->zrevrangebyscore ( 'letters', '+inf', '-inf' ) );
	}
	...
	public function testExclusiveScoreIntervals() {
		...
		$this->assertSame ( array (
				...
				'd',
				'c' 
		), $redis->zrevrangebyscore ( 'letters', '(30', 10 ) );
		$this->assertSame ( array (
				'f',
				'e',
				'd' 
	...
	public function testRangeWithWithscoresModifier() {
		...
		$this->assertSame ( $expected, $redis->zrevrangebyscore ( 'letters', 20, 10, array (
				'withscores' => true 
		) ) );
	}
