...
class RedisClusterHashStrategyTest extends PredisTestCase {
	...
	public function testKeysForScriptedCommand() {
		...
		$command->expects ( $this->once () )->method ( 'getScript' )->will ( $this->returnValue ( 'return true' ) );
		$command->expects ( $this->exactly ( 2 ) )->method ( 'getKeysCount' )->will ( $this->returnValue ( 1 ) );
	...
	public function testUnsettingCommandHandler() {
		...
		$command = $profile->createCommand ( 'get', array (
				'key' 
		) );
		$this->assertNull ( $strategy->getHash ( $command ) );
	}
