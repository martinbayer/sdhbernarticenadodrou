...
class PubSubContextTest extends PredisTestCase {
	...
	public function testPubSubContextRequirePubSubRelatedCommand() {
		...
		$profile->expects ( $this->any () )->method ( 'supportsCommands' )->will ( $this->returnValue ( false ) );
	...
	public function testConstructorWithSubscriptionsOpensContext() {
		...
		$cmdSubscribe = $profile->createCommand ( 'subscribe', array (
				'channel:foo' 
		) );
		$cmdPsubscribe = $profile->createCommand ( 'psubscribe', array (
				'channels:*' 
		) );
		...
		$client->expects ( $this->exactly ( 2 ) )->method ( 'createCommand' )->with ( $this->logicalOr ( $this->equalTo ( 'subscribe' ), $this->equalTo ( 'psubscribe' ) ) )->will ( $this->returnCallback ( function ($id, $args) use($profile) {
			return $profile->createCommand ( $id, $args );
		} ) );
		
		$options = array (
				'subscribe' => 'channel:foo',
				'psubscribe' => 'channels:*' 
		);
	...
	public function testClosingContextWithTrueClosesConnection() {
		...
		$pubsub = new PubSubContext ( $client, array (
				'subscribe' => 'channel:foo' 
		) );
