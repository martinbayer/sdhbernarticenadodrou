...
class DispatcherLoopTest extends PredisTestCase {
	...
	public function testDispatcherLoopAgainstRedisServer() {
		...
		$function01 = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
		...
		$function02 = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
		$function02->expects ( $this->once () )->method ( '__invoke' )->with ( '02:argument' );
		
		$function03 = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
	...
	public function testDispatcherLoopAgainstRedisServerWithPrefix() {
		...
		$producerPfx = new Client ( $parameters, $options + array (
				'prefix' => 'foobar' 
		) );
		$producerPfx->connect ();
		
		$consumer = new Client ( $parameters, $options + array (
				'prefix' => 'foobar' 
		) );
		$dispatcher = new DispatcherLoop ( $consumer );
		
		$callback = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
