...
class ConnectionFactoryTest extends PredisTestCase {
	...
	public function testCreateConnectionWithArrayParameters() {
		$factory = new ConnectionFactory ();
		$connection = $factory->create ( array (
				'scheme' => 'tcp',
				'custom' => 'foobar' 
		) );
		$parameters = $connection->getParameters ();
	...
	public function testCreateConnectionWithInitializationCommands() {
		...
		$profile = $this->getMock ( 'Predis\Profile\ServerProfileInterface' );
		$profile->expects ( $this->exactly ( 2 ) )->method ( 'createCommand' )->with ( $this->isType ( 'string' ), $this->isType ( 'array' ) )->will ( $this->returnCallback ( $createCommand ) );
	...
	public function testCreateConnectionWithDatabaseAndPasswordButNoProfile() {
		...
		$connection->expects ( $this->never () )->method ( 'getParameters' )->will ( $this->returnValue ( $parameters ) );
		$connection->expects ( $this->never () )->method ( 'pushInitCommand' );
	...
	public function testCreateUndefinedConnection() {
		...
		$factory = new ConnectionFactory ();
		$factory->create ( new ConnectionParameters ( array (
				'scheme' => $scheme 
		) ) );
	}
	...
	public function testDefineConnectionWithCallable() {
		...
		$initializerMock = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
		$initializerMock->expects ( $this->exactly ( 2 ) )->method ( '__invoke' )->with ( $parameters )->will ( $this->returnCallback ( $initializer ) );
	...
	public function testAggregatedConnectionSkipCreationOnConnectionInstance() {
		...
		$cluster->expects ( $this->exactly ( 2 ) )->method ( 'add' )->with ( $this->isInstanceOf ( 'Predis\Connection\SingleConnectionInterface' ) );
		
		$factory = $this->getMock ( 'Predis\Connection\ConnectionFactory', array (
				'create' 
		) );
	...
	public function testAggregatedConnectionWithMixedParameters() {
		...
		$cluster = $this->getMock ( 'Predis\Connection\ClusterConnectionInterface' );
		$cluster->expects ( $this->exactly ( 4 ) )->method ( 'add' )->with ( $this->isInstanceOf ( 'Predis\Connection\SingleConnectionInterface' ) );
		
		$factory = $this->getMock ( 'Predis\Connection\ConnectionFactory', array (
				'create' 
		) );
		$factory->expects ( $this->exactly ( 3 ) )->method ( 'create' )->will ( $this->returnCallback ( function ($_) use($connectionClass) {
	...
	public function testAggregatedConnectionWithServerProfileArgument() {
		...
		$factory->expects ( $this->exactly ( 2 ) )->method ( 'create' )->with ( $this->anything () )->will ( $this->returnCallback ( function ($_) use($connectionClass) {
			return new $connectionClass ();
		} ) );
		
		$nodes = array (
				'tcp://127.0.0.1:7001?password=foo',
				'tcp://127.0.0.1:7002?password=bar' 
		);
