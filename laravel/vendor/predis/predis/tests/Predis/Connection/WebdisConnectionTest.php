...
class WebdisConnectionTest extends PredisTestCase {
	...
	public function testRejectCommandSelect() {
		$connection = new WebdisConnection ( $this->getParameters () );
		$connection->executeCommand ( $this->getProfile ()->createCommand ( 'select', array (
				0 
		) ) );
	}
	...
	public function testRejectCommandAuth() {
		$connection = new WebdisConnection ( $this->getParameters () );
		$connection->executeCommand ( $this->getProfile ()->createCommand ( 'auth', array (
				'foobar' 
		) ) );
	}
	...
	public function testExecutesCommandsOnServer() {
		...
		$cmdEcho = $profile->createCommand ( 'echo', array (
				'echoed' 
		) );
		$cmdGet = $profile->createCommand ( 'get', array (
				'foobar' 
		) );
		$cmdRpush = $profile->createCommand ( 'rpush', array (
				'metavars',
				'foo',
				'hoge',
				'lol' 
		) );
		$cmdLrange = $profile->createCommand ( 'lrange', array (
				'metavars',
				0,
				- 1 
		) );
		...
		$this->assertSame ( array (
				...
				'hoge',
				'lol' 
		), $connection->executeCommand ( $cmdLrange ) );
	}
