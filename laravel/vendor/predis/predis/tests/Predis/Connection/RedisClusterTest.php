...
class RedisClusterTest extends PredisTestCase {
	...
	public function testIsConnectedReturnsTrueIfAtLeastOneConnectionIsOpen() {
		...
		$connection2 = $this->getMockConnection ( 'tcp://127.0.0.1:6380' );
		$connection2->expects ( $this->once () )->method ( 'isConnected' )->will ( $this->returnValue ( true ) );
	...
	public function testReturnsCorrectConnectionUsingCommandInstance() {
		...
		$set = $profile->createCommand ( 'set', array (
				'node:1001',
				'foobar' 
		) );
		$get = $profile->createCommand ( 'get', array (
				'node:1001' 
		) );
	...
	public function testWritesCommandToCorrectConnection() {
		$command = ServerProfile::getDefault ()->createCommand ( 'get', array (
				'node:1001' 
		) );
	...
	public function testAskResponseWithConnectionInPool() {
		...
		$connection1 = $this->getMockConnection ( 'tcp://127.0.0.1:6379' );
		$connection1->expects ( $this->exactly ( 2 ) )->method ( 'executeCommand' )->with ( $command )->will ( $this->onConsecutiveCalls ( $askResponse, 'foobar' ) );
		
		$connection2 = $this->getMockConnection ( 'tcp://127.0.0.1:6380' );
		$connection2->expects ( $this->at ( 2 ) )->method ( 'executeCommand' )->with ( $this->isRedisCommand ( 'ASKING' ) );
		$connection2->expects ( $this->at ( 3 ) )->method ( 'executeCommand' )->with ( $command )->will ( $this->returnValue ( 'foobar' ) );
	...
	public function testAskResponseWithConnectionNotInPool() {
		...
		$connection3 = $this->getMockConnection ( 'tcp://127.0.0.1:6381' );
		$connection3->expects ( $this->at ( 0 ) )->method ( 'executeCommand' )->with ( $this->isRedisCommand ( 'ASKING' ) );
		$connection3->expects ( $this->at ( 1 ) )->method ( 'executeCommand' )->with ( $command )->will ( $this->returnValue ( 'foobar' ) );
		
		$factory = $this->getMock ( 'Predis\Connection\ConnectionFactory' );
		$factory->expects ( $this->once () )->method ( 'create' )->with ( array (
				'host' => '127.0.0.1',
				'port' => '6381' 
	...
	public function testMovedResponseWithConnectionInPool() {
		...
		$connection1 = $this->getMockConnection ( 'tcp://127.0.0.1:6379' );
		$connection1->expects ( $this->exactly ( 1 ) )->method ( 'executeCommand' )->with ( $command )->will ( $this->returnValue ( $movedResponse ) );
		
		$connection2 = $this->getMockConnection ( 'tcp://127.0.0.1:6380' );
		$connection2->expects ( $this->exactly ( 2 ) )->method ( 'executeCommand' )->with ( $command )->will ( $this->onConsecutiveCalls ( 'foobar', 'foobar' ) );
	...
	public function testFetchSlotsMapFromClusterWithClusterSlotsCommand() {
		...
		$connection1->expects ( $this->once () )->method ( 'executeCommand' )->with ( $command )->will ( $this->returnValue ( $response ) );
	...
	public function testAskSlotsMapToRedisClusterOnMovedResponseByDefault() {
		...
		$connection2 = $this->getMockConnection ( 'tcp://127.0.0.1:6380' );
		$connection2->expects ( $this->at ( 0 ) )->method ( 'executeCommand' )->with ( $this->isRedisCommand ( 'CLUSTER', array (
				'SLOTS' 
