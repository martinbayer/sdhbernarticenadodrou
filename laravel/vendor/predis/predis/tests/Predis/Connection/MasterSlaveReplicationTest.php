...
class MasterSlaveReplicationTest extends PredisTestCase {
	...
	public function testUsesSlavesOnReadOnlyCommands() {
		...
		$cmd = $profile->createCommand ( 'exists', array (
				'foo' 
		) );
		$this->assertSame ( $slave1, $replication->getConnection ( $cmd ) );
		
		$cmd = $profile->createCommand ( 'get', array (
				'foo' 
		) );
		$this->assertSame ( $slave1, $replication->getConnection ( $cmd ) );
	}
	...
	public function testUsesMasterOnWriteCommands() {
		...
		$cmd = $profile->createCommand ( 'set', array (
				'foo',
				'bar' 
		) );
	...
	public function testWritesCommandToCorrectConnection() {
		$profile = ServerProfile::getDefault ();
		$cmdExists = $profile->createCommand ( 'exists', array (
				'foo' 
		) );
		$cmdSet = $profile->getDefault ()->createCommand ( 'set', array (
				'foo',
				'bar' 
		) );
	...
	public function testWatchTriggersSwitchToMasterConnection() {
		$profile = ServerProfile::getDefault ();
		$cmdWatch = $profile->createCommand ( 'watch', array (
				'foo' 
		) );
	...
	public function testEvalTriggersSwitchToMasterConnection() {
		...
		$cmdEval = $profile->createCommand ( 'eval', array (
	...
	public function testSortTriggersSwitchToMasterConnectionOnStoreModifier() {
		$profile = ServerProfile::get ( 'dev' );
		$cmdSortNormal = $profile->createCommand ( 'sort', array (
				'key' 
		) );
		$cmdSortStore = $profile->createCommand ( 'sort', array (
				'key',
				array (
						'store' => 'key:store' 
				) 
	...
	public function testCanOverrideReadOnlyFlagForCommands() {
		$profile = ServerProfile::getDefault ();
		$cmdSet = $profile->createCommand ( 'set', array (
				'foo',
				'bar' 
		) );
		$cmdGet = $profile->createCommand ( 'get', array (
				'foo' 
		) );
	...
	public function testAcceptsCallableToOverrideReadOnlyFlagForCommands() {
		$profile = ServerProfile::getDefault ();
		$cmdExistsFoo = $profile->createCommand ( 'exists', array (
				'foo' 
		) );
		$cmdExistsBar = $profile->createCommand ( 'exists', array (
				'bar' 
		) );
	...
	public function testCanSetReadOnlyFlagForEvalScripts() {
		...
		$cmdEval = $profile->createCommand ( 'eval', array (
				$script = "return redis.call('info');" 
		) );
		$cmdEvalSha = $profile->createCommand ( 'evalsha', array (
