...
class StandardExecutorTest extends PredisTestCase {
	...
	public function testExecutorWithSingleConnection() {
		...
		$connection->expects ( $this->exactly ( 3 ) )->method ( 'readResponse' )->will ( $this->returnValue ( 'PONG' ) );
	...
	public function testExecutorCanThrowExceptions() {
		...
		$connection = $this->getMock ( 'Predis\Connection\SingleConnectionInterface' );
		$connection->expects ( $this->once () )->method ( 'readResponse' )->will ( $this->returnValue ( $error ) );
	...
	protected function simpleResponseObjectTest(PipelineExecutorInterface $executor, ResponseObjectInterface $response) {
		...
		$command->expects ( $this->never () )->method ( 'parseResponse' );
		
		$connection = $this->getMock ( 'Predis\Connection\SingleConnectionInterface' );
		$connection->expects ( $this->once () )->method ( 'writeCommand' );
		$connection->expects ( $this->once () )->method ( 'readResponse' )->will ( $this->returnValue ( $response ) );
