...
class ReplicationStrategyTest extends PredisTestCase {
	...
	public function testSortCommand() {
		...
		$cmdReadSort = $profile->createCommand ( 'SORT', array (
				'key:list' 
		) );
		$this->assertTrue ( $strategy->isReadOperation ( $cmdReadSort ), 'SORT [read-only]' );
		
		$cmdWriteSort = $profile->createCommand ( 'SORT', array (
				'key:list',
				array (
						'store' => 'key:stored' 
				) 
	...
	public function testDefaultIsWriteOperation() {
		...
		$command->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( 'CMDTEST' ) );
	...
	public function testCanUseCallableToCheckCommand() {
		...
		$command = $profile->createCommand ( 'SET', array (
				'trigger',
				false 
		) );
		$this->assertFalse ( $strategy->isReadOperation ( $command ) );
	 ...
	 *
	 * @param string $type
	 *        	Optional type of command (based on its keys)
	 * @return array
	 */
