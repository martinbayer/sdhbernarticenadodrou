...
class ResponseBulkHandlerTest extends PredisTestCase {
	...
	public function testZeroLengthBulk() {
		...
		$connection->expects ( $this->once () )->method ( 'readBytes' )->with ( $this->equalTo ( 2 ) )->will ( $this->returnValue ( "\r\n" ) );
	...
	public function testBulk() {
		...
		$connection->expects ( $this->never () )->method ( 'readLine' );
		$connection->expects ( $this->once () )->method ( 'readBytes' )->with ( $this->equalTo ( $bulkLengh + 2 ) )->will ( $this->returnValue ( "$bulk\r\n" ) );
