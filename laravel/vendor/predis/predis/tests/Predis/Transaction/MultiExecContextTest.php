...
class MultiExecContextTest extends PredisTestCase {
	...
	public function testThrowsExceptionOnUnsupportedMultiExecInProfile() {
		...
		$profile->expects ( $this->once () )->method ( 'supportsCommands' )->with ( array (
				'MULTI',
				'EXEC',
				'DISCARD' 
	...
	public function testThrowsExceptionOnUnsupportedWatchUnwatchInProfile() {
		$profile = $this->getMock ( 'Predis\Profile\ServerProfileInterface' );
		$profile->expects ( $this->at ( 0 ) )->method ( 'supportsCommands' )->with ( array (
				'MULTI',
				'EXEC',
				'DISCARD' 
		) )->will ( $this->returnValue ( true ) );
		$profile->expects ( $this->at ( 1 ) )->method ( 'supportsCommands' )->with ( array (
				'WATCH',
				'UNWATCH' 
		) )->will ( $this->returnValue ( false ) );
		...
		$tx = new MultiExecContext ( $client, array (
				'options' => 'cas' 
		) );
		$tx->watch ( 'foo' );
	}
	...
	public function testAutomaticWatchInOptions() {
		...
		$this->assertSame ( array (
				'WATCH' 
		), self::commandsToIDs ( $casCommands ) );
		$this->assertSame ( array (
				'foo',
				'hoge' 
		), $casCommands [0]->getArguments () );
		$this->assertSame ( array (
				'MULTI',
				'GET',
				'GET',
				'EXEC' 
		), self::commandsToIDs ( $txCommands ) );
	}
	/**
	 * @group disconnected
	 */
	public function testCheckAndSetWithFluentInterface() {
		...
		$replies = $tx->multi ()->get ( 'foo' )->get ( 'hoge' )->execute ();
	...
	public function testAutomaticRetryOnServerSideTransactionAbort() {
		...
		$sentinel = $this->getMock ( 'stdClass', array (
				'signal' 
		) );
	...
	public function testIntegrationReturnsErrorObjectOnRedisErrorInBlock() {
		$client = $this->getClient ( array (), array (
				'exceptions' => false 
		) );
	...
	public function testIntegrationWritesOnWatchedKeysAbortTransaction() {
		...
		try {
			$client1->multiExec ( array (
					'watch' => 'sentinel' 
			), function ($tx) use($client2) {
	...
	protected function getMockedConnection($executeCallback) {
		$connection = $this->getMock ( 'Predis\Connection\SingleConnectionInterface' );
		$connection->expects ( $this->any () )->method ( 'executeCommand' )->will ( $this->returnCallback ( $executeCallback ) );
	 ...
	 *        	Reference to an array storing the whole flow of commands.
	 * @param array $cas
	 *        	Check and set operations performed by the transaction.
	 * @return \Closure
	 */
	 ...
	 *
	 * @param
	 *        	array Additional connection parameters.
	 * @param
	 *        	array Additional client options.
