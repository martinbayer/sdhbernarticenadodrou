...
class ServerProfileTest extends PredisTestCase {
	...
	public function testSupportCommands() {
		...
		$this->assertTrue ( $profile->supportsCommands ( array (
				'get',
				'set' 
		) ) );
		$this->assertTrue ( $profile->supportsCommands ( array (
		...
		$this->assertFalse ( $profile->supportsCommands ( array (
				'get',
				'unknown' 
		) ) );
	...
	public function testSingleProcessor() {
		...
		$processor = $this->getMock ( 'Predis\Command\Processor\CommandProcessorInterface' );
		$processor->expects ( $this->once () )->method ( 'process' )->with ( $this->isInstanceOf ( 'Predis\Command\CommandInterface' ) )->will ( $this->returnCallback ( function ($cmd) use(&$argsRef) {
