...
class ClientTest extends PredisTestCase {
	...
	public function testConstructorWithReplicationArgument() {
		...
		$factory = new ConnectionFactory ();
		$factory->createAggregated ( $replication, array (
				'tcp://host1?alias=master',
				'tcp://host2?alias=slave' 
		) );
	...
	public function testConstructorWithCallableArgument() {
		...
		$callable = $this->getMock ( 'stdClass', array (
				'__invoke' 
		) );
		$callable->expects ( $this->once () )->method ( '__invoke' )->with ( $this->isInstanceOf ( 'Predis\Option\ClientOptions' ) )->will ( $this->returnValue ( $connection ) );
	...
	public function testConstructorWithCallableArgumentButInvalidReturnType() {
		...
		$callable->expects ( $this->once () )->method ( '__invoke' )->with ( $this->isInstanceOf ( 'Predis\Option\ClientOptions' ) )->will ( $this->returnValue ( $wrongType ) );
	...
	public function testExecuteCommandReturnsParsedReplies() {
		...
		$hgetall = $profile->createCommand ( 'hgetall', array (
				'metavars',
				'foo',
				'hoge' 
		) );
		...
		$connection = $this->getMock ( 'Predis\Connection\ConnectionInterface' );
		$connection->expects ( $this->at ( 0 ) )->method ( 'executeCommand' )->with ( $ping )->will ( $this->returnValue ( 'PONG' ) );
		$connection->expects ( $this->at ( 1 ) )->method ( 'executeCommand' )->with ( $hgetall )->will ( $this->returnValue ( array (
				'foo',
				'bar',
				'hoge',
				'piyo' 
	...
	public function testExecuteCommandThrowsExceptionOnRedisError() {
		...
		$connection = $this->getMock ( 'Predis\Connection\ConnectionInterface' );
		$connection->expects ( $this->once () )->method ( 'executeCommand' )->will ( $this->returnValue ( $expectedResponse ) );
	...
	public function testExecuteCommandReturnsErrorResponseOnRedisError() {
		...
		$client = new Client ( $connection, array (
				'exceptions' => false 
		) );
	...
	public function testCallingRedisCommandExecutesInstanceOfCommand() {
		...
		$client = $this->getMock ( 'Predis\Client', null, array (
				$connection,
				$options 
		) );
	...
	public function testGetConnectionFromAggregatedConnectionWithAlias() {
		$client = new Client ( array (
				'tcp://host1?alias=node01',
				'tcp://host2?alias=node02' 
		) );
	...
	public function testGetClientForReturnsInstanceOfSubclass() {
		...
		$client = $this->getMock ( 'Predis\Client', array (
				'dummy' 
		), array (
				$nodes 
		), 'SubclassedClient' );
	...
	public function testPipelineWithArrayReturnsPipelineContextWithOptions() {
		...
		$options = array (
				'executor' => function ($client, $options) use($executor) {
					return $executor;
				} 
		);
		$this->assertInstanceOf ( 'Predis\Pipeline\PipelineContext', $pipeline = $client->pipeline ( $options ) );
	...
	public function testPipelineWithCallableExecutesPipeline() {
		...
		$callable->expects ( $this->once () )->method ( '__invoke' )->with ( $this->isInstanceOf ( 'Predis\Pipeline\PipelineContext' ) );
	...
	public function testClientResendScriptedCommandUsingEvalOnNoScriptErrors() {
		$command = $this->getMockForAbstractClass ( 'Predis\Command\ScriptedCommand', array (), '', true, true, true, array (
				'parseResponse' 
		) );
