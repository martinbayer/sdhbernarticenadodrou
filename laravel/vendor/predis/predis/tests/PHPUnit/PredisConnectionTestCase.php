...
abstract class PredisConnectionTestCase extends PredisTestCase {
	...
	public function testExecutesCommandOnServer() {
		...
		$cmdPing = $this->getMock ( $profile->getCommandClass ( 'ping' ), array (
				'parseResponse' 
		) );
		$cmdPing->expects ( $this->never () )->method ( 'parseResponse' );
	...
	public function testIsAbleToWriteMultipleCommandsAndReadThemBackForPipelining() {
		...
		$cmdEcho = $this->getMock ( $profile->getCommandClass ( 'echo' ), array (
				'parseResponse' 
		) );
		$cmdEcho->setArguments ( array (
				'ECHOED' 
		) );
		$cmdEcho->expects ( $this->never () )->method ( 'parseResponse' );
	...
	public function testSendsInitializationCommandsOnConnection() {
		...
		$cmdPing->expects ( $this->once () )->method ( 'getArguments' )->will ( $this->returnValue ( array () ) );
		
		$cmdEcho = $this->getMock ( $profile->getCommandClass ( 'echo' ), array (
				'getArguments' 
		) );
		$cmdEcho->expects ( $this->once () )->method ( 'getArguments' )->will ( $this->returnValue ( array (
				'ECHOED' 
		) ) );
	...
	public function testReadsStatusReplies() {
		...
		$connection->writeCommand ( $profile->createCommand ( 'set', array (
				'foo',
				'bar' 
		) ) );
	...
	public function testReadsBulkReplies() {
		...
		$connection->executeCommand ( $profile->createCommand ( 'set', array (
				'foo',
				'bar' 
		) ) );
		
		$connection->writeCommand ( $profile->createCommand ( 'get', array (
				'foo' 
		) ) );
	...
	public function testReadsIntegerReplies() {
		...
		$connection->writeCommand ( $profile->createCommand ( 'llen', array (
				'metavars' 
		) ) );
	...
	public function testReadsErrorRepliesAsResponseErrorObjects() {
		...
		$connection->writeCommand ( $profile->createCommand ( 'rpush', array (
				'foo',
				'baz' 
		) ) );
	...
	public function testThrowsExceptionOnConnectionTimeout() {
		$connection = $this->getConnection ( $_, false, array (
				'host' => '169.254.10.10',
				'timeout' => 0.5 
		) );
	...
	public function testThrowsExceptionOnReadWriteTimeout() {
		$connection = $this->getConnection ( $profile, true, array (
				'read_write_timeout' => 0.5 
		) );
		
		$connection->executeCommand ( $profile->createCommand ( 'brpop', array (
				'foo',
				3 
		) ) );
	}
	 ...
	 *        	Reference to the server profile instance.
	 * @param bool $initialize
	 *        	Push default initialization commands (SELECT and FLUSHDB).
