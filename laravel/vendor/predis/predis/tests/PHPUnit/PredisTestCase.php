...
abstract class PredisTestCase extends PHPUnit_Framework_TestCase {
	 ...
	 *        	Expected command or command ID.
	 ...
	 *
	 * @param array|string|CommandInterface $expected
	 *        	Expected command.
	 * @param mixed $actual
	 *        	Actual command.
	 ...
	 * @param bool $flushdb
	 *        	Flush selected database before returning the client.
	 * @return Client
	 */
	...
	protected function setRequiredRedisVersionFromAnnotation() {
		...
		if (isset ( $annotations ['method'] ['requiresRedisVersion'], $annotations ['method'] ['group'] ) && ! empty ( $annotations ['method'] ['requiresRedisVersion'] ) && in_array ( 'connected', $annotations ['method'] ['group'] )) {
	...
	protected function checkRequiredRedisVersion() {
		...
		if (! $match = eval ( "return $comparation $expOperator 0;" )) {
			$this->markTestSkipped ( "This test requires Redis $expOperator $expVersion but the current version is $srvVersion." );
		}
	}
