...
class StackedHttpKernelTest extends \PHPUnit_Framework_TestCase {
	...
	private function getHttpKernelMock(Response $response) {
		...
		$app->expects ( $this->any () )->method ( 'handle' )->with ( $this->isInstanceOf ( 'Symfony\Component\HttpFoundation\Request' ) )->will ( $this->returnValue ( $response ) );
	...
	private function getTerminableMock(Response $response = null) {
		...
		$app->expects ( $this->once () )->method ( 'terminate' )->with ( $this->isInstanceOf ( 'Symfony\Component\HttpFoundation\Request' ), $this->isInstanceOf ( 'Symfony\Component\HttpFoundation\Response' ) );
		
		return $app;
	}
	private function getDelegatingTerminableMock(TerminableInterface $next) {
		$app = $this->getMock ( 'Stack\TerminableHttpKernel' );
		$app->expects ( $this->once () )->method ( 'terminate' )->with ( $this->isInstanceOf ( 'Symfony\Component\HttpFoundation\Request' ), $this->isInstanceOf ( 'Symfony\Component\HttpFoundation\Response' ) )->will ( $this->returnCallback ( function ($request, $response) use($next) {
