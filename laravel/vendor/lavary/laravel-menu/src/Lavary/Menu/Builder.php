...
class Builder {
	...
	public function divide(array $attributes = array()) {
		$attributes ['class'] = self::formatGroupClass ( array (
				'class' => 'divider' 
		), $attributes );
	...
	protected static function mergeGroup($new, $old) {
		...
		return array_merge ( array_except ( $old, array (
				'prefix',
				'class' 
		) ), $new );
	}
	...
	public static function mergeStatic($new = null, array $old = array()) {
		...
		return \HTML::attributes ( array_merge ( array_except ( $old, array (
				'class' 
		) ), $attrs ) );
	}
