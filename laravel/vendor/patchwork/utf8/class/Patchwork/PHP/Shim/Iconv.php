...
class Iconv {
	...
	ERROR_ILLEGAL_CHARACTER = 'iconv(): Detected an illegal character in input string', ERROR_WRONG_CHARSET = 'iconv(): Wrong charset, conversion from `%s\' to `%s\' is not allowed';
	public static 

	$input_encoding = 'utf-8', $output_encoding = 'utf-8', $internal_encoding = 'utf-8';
	protected static 
	...
	$translit_map = array (), $convert_map = array (), $error_handler, $last_error, 
	...
	static function iconv($in_charset, $out_charset, $str) {
		...
		if (('utf-8' !== $in_charset && ! static::loadMap ( 'from.', $in_charset, $in_map )) || ('utf-8' !== $out_charset && ! static::loadMap ( 'to.', $out_charset, $out_map ))) {
		...
		foreach ( $str as $str ) {
			...
			if (2 === count ( $str )) {
				if (isset ( $headers [$str [0]] )) {
					is_array ( $headers [$str [0]] ) || $headers [$str [0]] = array (
							$headers [$str [0]] 
					);
	...
	static function iconv_mime_decode($str, $mode = 0, $charset = INF) {
		...
		while ( $i < $len ) {
			$c = strtolower ( $str [$i] );
			if ((ICONV_MIME_DECODE_CONTINUE_ON_ERROR & $mode) && 'utf-8' !== $c && ! isset ( self::$alias [$c] ) && ! static::loadMap ( 'from.', $c, $d ))
				$d = false;
	...
	static function iconv_mime_encode($field_name, $field_value, $pref = INF) {
		...
		foreach ( $chars as $c ) {
			...
			$o = $Q ? $c = preg_replace_callback ( '/[=_\?\x00-\x1F\x80-\xFF]/', array (
					__CLASS__,
					'qp_byte_callback' 
			), $c ) : base64_encode ( $line_data . $c );
