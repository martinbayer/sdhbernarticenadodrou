...
class Mbstring {
	...
	$encoding_list = array (
			'ASCII',
			'UTF-8' 
	), $language = 'neutral', $internal_encoding = 'UTF-8', $caseFold = array (
			array (
					'µ',
					'ſ',
					"\xCD\x85",
	...
	static function mb_convert_encoding($s, $to_encoding, $from_encoding = INF) {
		...
		if ('html-entities' === $to_encoding) {
			...
			'utf-8' === $from_encoding || 'utf8' === $from_encoding || $s = iconv ( $from_encoding, 'UTF-8//IGNORE', $s );
	...
	static function mb_check_encoding($var = INF, $encoding = INF) {
		...
		return false !== mb_detect_encoding ( $var, array (
				$encoding 
		), true );
	}
